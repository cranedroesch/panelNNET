  ...
  if (is.null(newX)){
    ...
    if (!is.null(obj$fe)){
      ...
      if (any(unique(fe.newX) %ni% unique(obj$fe$fe_var)) & !is.null(y_test)){
        ...
        nlayers <- nlayers[(!grepl("param", names(nlayers)))]-1 #subtrack off one because the parlist has one more element than the number of layers
...
predfun_multinet <- function(plist, obj, newX = NULL, fe.newX = NULL, new.param = NULL,
  ...
  nlayers <- sapply(obj$parlist, length)
  nlayers <- nlayers[(!grepl("param", names(nlayers)))] -1
  # rescale new data to scale of training data
  D <- foreach(i = 1:length(obj$X)) %do% {
